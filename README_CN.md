...
- 🌓 **主题支持** - 自动检测或强制亮/暗色模式
- 📐 **响应式设计** - 自动调整大小
- ↔️ **拖拽调宽** - 拖动块两侧手柄调整宽度，通过 `width=N` 围栏参数按块持久化
- 🎯 **居中对齐** - 从工具栏一键居中，或通过 `align=center` 参数，也可在设置中开启「默认居中」
- 🔄 **实时刷新** - 单个命令刷新所有信息图表
...
- `width=N` - 固定块宽度（像素），例如 <code>```infographic width=600</code>。也可以直接拖动渲染块两侧的手柄调整宽度；在源码模式下编辑时，宽度会自动写回围栏参数。
- `align=center` / `align=left` - 水平对齐方式，覆盖设置中的「默认居中」。也可以通过工具栏的 **Center** 按钮切换。

...
| Dev deploy | set `OBSIDIAN_VAULT` in `.env.local` (gitignored) | Every build auto-copies `main.js`/`manifest.json`/`styles.css` to `<vault>/.obsidian/plugins/infographic` |
...
- **Dev vault deploy + CLI reload**: with `OBSIDIAN_VAULT` configured, builds deploy into the vault automatically; reload the running plugin with the Obsidian CLI (`obsidian plugin:reload id=infographic`, probe with `obsidian eval`). First-time install of a new plugin id needs `obsidian eval 'code=(async () => { await app.plugins.loadManifests(); await app.plugins.enablePluginAndSave("infographic") })()'`.
